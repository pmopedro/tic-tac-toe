...
from robot_player import RobotTicTacToePlayer
import pygame
    ...
    def move(self, pos):
        try:
            ...
            if self.table[x][y] == "-":
                ...
                pygame.display.flip()
                self.change_player()
                r = RobotTicTacToePlayer(player=self.player)
                move = r.make_move(self.table)[1]
                (x, y) = move
                self.table[x][y] = self.player
                self.draw_char(x, y, self.player)
                self.game_check()
    ...
    def pattern_strike(self, start_point, end_point, line_type):
        ...
        if line_type == "ver":
            start_x, start_y = start_point[0] * \
                self.cell_size + mid_val, self.table_space
            end_x, end_y = end_point[0] * self.cell_size + \
                mid_val, self.table_size - self.table_space

        # for the horizontal winning pattern
        elif line_type == "hor":
            start_x, start_y = self.table_space, start_point[-1] * \
                self.cell_size + mid_val
            end_x, end_y = self.table_size - \
                self.table_space, end_point[-1] * self.cell_size + mid_val
        ...
        # for the diagonal winning pattern from top-left to bottom right
            ...
            end_x, end_y = self.table_size - \
                self.table_space, self.table_size - self.table_space
        ...
        line_strike = pygame.draw.line(screen, self.line_color, [
